...
def get_gas_costs_per_dist(airdrops_per_month: Dict[int, int]):
    gas_costs_per_dist = {dist_id: airdrops * TOTAL_GAS_COST_AIRDROP for dist_id, airdrops in airdrops_per_month.items()}
...
def get_gas_costs_per_dist_compressed(txs: List[List[dict]]) -> dict:
    ...
    for month_id, amounts_per_month in enumerate(amounts_for_all_months):
        gas_costs_per_dist_compressed[month_id] = same_amount_monthly(amounts_per_month)
...
def same_amount_monthly(month_amounts: List[float]) -> int:
    amount_occurrences = get_unique_amounts_per_distribution(month_amounts)
    ...
    for karma, num_repeats in amount_occurrences.items():
        ...
        total_bits_for_amount = BITS_USER_BALANCE
...
def make_stats_compression(gas_costs_per_dist: Dict[int, int], gas_cost_per_dist_same_amounts: Dict[int, int], gas_cost_per_dist_permutations: Dict[int, int], gas_cost_per_dist_referencing: Dict[int, int]) -> Dict[int, Dict[str, Union[int, float]]]:
    ...
    for month_id, gas_cost_per_dist in gas_costs_per_dist.items():
        ...
        gas_cost_stats[month_id]['savings_naive_v_same_amounts'] = gas_cost_stats[month_id]['naive'] - gas_cost_stats[month_id]['same_amounts']
        gas_cost_stats[month_id]['savings_percent_naive_v_same_amounts'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_same_amounts']/gas_cost_stats[month_id]['naive'])

        gas_cost_stats[month_id]['savings_naive_v_permutations'] = gas_cost_stats[month_id]['naive'] - gas_cost_stats[month_id]['permutations']
        gas_cost_stats[month_id]['savings_percent_naive_v_permutations'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_permutations']/gas_cost_stats[month_id]['naive'])

        gas_cost_stats[month_id]['savings_naive_v_referencing'] = gas_cost_stats[month_id]['naive'] - gas_cost_stats[month_id]['referencing']
        gas_cost_stats[month_id]['savings_percent_naive_v_referencing'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_referencing']/gas_cost_stats[month_id]['naive'])

        if month_id != 0:
            gas_cost_stats[month_id]['naive_cumulative'] = gas_cost_stats[month_id - 1]['naive_cumulative'] + gas_cost_per_dist
            gas_cost_stats[month_id]['same_amounts_cumulative'] = gas_cost_stats[month_id - 1]['same_amounts_cumulative'] + gas_cost_per_dist_same_amounts[month_id]
            gas_cost_stats[month_id]['permutations_cumulative'] = gas_cost_stats[month_id - 1]['permutations_cumulative'] + gas_cost_per_dist_permutations[month_id]
            gas_cost_stats[month_id]['referencing_cumulative'] = gas_cost_stats[month_id - 1]['referencing_cumulative'] + gas_cost_per_dist_referencing[month_id]
        else:
        ...
        gas_cost_stats[month_id]['savings_naive_v_same_amounts_cumulative'] = gas_cost_stats[month_id]['naive_cumulative'] - gas_cost_stats[month_id]['same_amounts_cumulative']
        gas_cost_stats[month_id]['savings_percent_naive_v_same_amounts_cumulative'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_same_amounts_cumulative']/gas_cost_stats[month_id]['naive_cumulative'])

        gas_cost_stats[month_id]['savings_naive_v_permutations_cumulative'] = gas_cost_stats[month_id]['naive_cumulative'] - gas_cost_stats[month_id]['permutations_cumulative']
        gas_cost_stats[month_id]['savings_percent_naive_v_permutations_cumulative'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_permutations_cumulative']/gas_cost_stats[month_id]['naive_cumulative'])

        gas_cost_stats[month_id]['savings_naive_v_referencing_cumulative'] = gas_cost_stats[month_id]['naive_cumulative'] - gas_cost_stats[month_id]['referencing_cumulative']
        gas_cost_stats[month_id]['savings_percent_naive_v_referencing_cumulative'] = 100 * (gas_cost_stats[month_id]['savings_naive_v_referencing_cumulative']/gas_cost_stats[month_id]['naive_cumulative'])
...
def print_compression_stats(stats: Dict[int, Dict[str, Union[int, float]]]):
    ...
    for dist_id, dist_stats in stats.items():
        ...
        print(f"Gas costs | Naive: {dist_stats['naive']}, Same amounts: {dist_stats['same_amounts']}, Permutations: {dist_stats['permutations']}, Referencing: {dist_stats['referencing']}")
        print(f"Savings naive v same amounts | Total:  {dist_stats['savings_naive_v_same_amounts']}, %: {dist_stats['savings_percent_naive_v_same_amounts']}")
        print(f"Savings naive v permutations | Total:  {dist_stats['savings_naive_v_permutations']}, %: {dist_stats['savings_percent_naive_v_permutations']}")
        print(f"Savings naive v referencing | Total:  {dist_stats['savings_naive_v_referencing']}, %: {dist_stats['savings_percent_naive_v_referencing']}\n")

        print(f"Cumulative")
        print(f"Gas costs | Naive: {dist_stats['naive_cumulative']}, Same amounts: {dist_stats['same_amounts_cumulative']}, Permutations: {dist_stats['permutations_cumulative']}, Referencing: {dist_stats['referencing_cumulative']}")
        print(f"Savings naive v same amounts | Total: {dist_stats['savings_naive_v_same_amounts_cumulative']}, %: {dist_stats['savings_percent_naive_v_same_amounts_cumulative']}")
        print(f"Savings naive v permutations | Total: {dist_stats['savings_naive_v_permutations_cumulative']}, %: {dist_stats['savings_percent_naive_v_permutations_cumulative']}")
        print(f"Savings naive v referencing | Total: {dist_stats['savings_naive_v_referencing_cumulative']}, %: {dist_stats['savings_percent_naive_v_referencing_cumulative']}\n")

    print("===========================================================================\n")

    last_dist_id = len(stats.keys()) - 1
    print("Total")
    print(f"Gas costs | Naive: {stats[last_dist_id]['naive_cumulative']}, Same amounts: {stats[last_dist_id]['same_amounts_cumulative']}, Permutations: {stats[last_dist_id]['permutations_cumulative']}, Referencing: {stats[last_dist_id]['referencing_cumulative']}")
    print(f"Savings naive v same amounts | Total: {stats[last_dist_id]['savings_naive_v_same_amounts_cumulative']}, %: {stats[last_dist_id]['savings_percent_naive_v_same_amounts_cumulative']}")
    print(f"Savings naive v permutations | Total: {stats[last_dist_id]['savings_naive_v_permutations_cumulative']}, %: {stats[last_dist_id]['savings_percent_naive_v_permutations_cumulative']}")
    print(f"Savings naive v referencing | Total: {stats[last_dist_id]['savings_naive_v_referencing_cumulative']}, %: {stats[last_dist_id]['savings_percent_naive_v_referencing_cumulative']}")
...
def get_data_stats(txs: List[List[dict]]):
    ...
    for distribution_id, distribution_unique_amounts in unique_amounts.items():
        unique_amounts_per_distribution_descending = {amount: times for amount, times in sorted(distribution_unique_amounts.items(), key=lambda item: item[1], reverse=True)}
        top_50_most_common = {amount: unique_amounts_per_distribution_descending[amount] for amount in list(unique_amounts_per_distribution_descending.keys())[:50]}
        print(f"Distribution: {distribution_id}, number of unique amounts: {len(distribution_unique_amounts)}, Most common: {top_50_most_common}")
...
def compute_compression_stats(txs: List[List[dict]], airdrops_per_month: Dict[int, int]):
    ...
    stats = make_stats_compression(gas_cost_per_dist_naive, gas_cost_per_dist_compressed, gas_cost_per_dist_permutations, gas_cost_per_dist_referencing)
...
def get_gas_costs_per_dist_permutations(txs: List[List[dict]]) -> Dict[int, int]:
    ...
    permutations_savings, permutation_costs, repeating_users_per_dist = get_stats_for_reusing_groups_all_data(txs, permutations, repeated_users)
...
def get_stats_for_reusing_groups_all_data(txs: List[List[dict]], permutations: Dict[str, int], repeated_users: List[str]) -> Tuple[List[int], List[int], List[int]]:
...
def get_gas_costs_for_reusing_groups_all_data(txs: List[List[dict]], permutations_costs_per_dist: List[int], repeating_users_per_dist: List[int]) -> Dict[int, int]:
...
def get_gas_cost_dist_referencing(parsed_data: dict) -> Dict[int, int]:
    ...
    for distribution_id, distribution_data in parsed_data.items():
        ...
        distribution_data['referencing_cost'] = num_unique_users_not_part_in_current_dist * BITS_PREV_REFERENCE * GAS_COST_BIT
        ...
        distribution_data['gas_cost'] = distribution_data['referencing_cost'] + distribution_data['repeating_users_cost'] + distribution_data['unique_users_cost']
    ...
    for distribution_id, distribution_data in parsed_data.items():
        ...
        print(f"Gas cost per distribution: {distribution_id}, GC: {distribution_data['gas_cost']}, RC: {distribution_data['referencing_cost']}, RUC: {distribution_data['repeating_users_cost']}, UUC: {distribution_data['unique_users_cost']}")


    print(f"Total: GC: {total_gas_cost}, RC: {total_referencing_cost}, RUC: {total_repeating_user_cost}, UUC: {total_unique_user_cost}")
    return gas_costs
